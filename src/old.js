...
const newdocument = new documentProject(
  "New-Document",
  "This is a new document"
);

const myMap = new Map([[newdocument.title, newdocument.content]]);
...
let mainTab = (
  ...
  <div className="flex-box">
    <div className="tab" id="tab">
      <CreateTab title="New-Document" children="New-Document"></CreateTab>
      <CreateTab title="New-Document1" children="New-Document1"></CreateTab>
    </div>
...
function CreateTab({ title, children }) {
  return (
    <button
      className={title + " tablinks active"}
      onClick={() => openDocument(title)}
    >
      {children}
    </button>
  );
...
function createDocument(props) {
  const newObject = new documentProject(
    "New-Document" + newDocIndex,
    "This is a new document"
  );
  newDocIndex++;
